...
const reducer = (state, action) => {
    ...
    switch (action.type) {
        ...
        case actionType.SETTING_SIZE_FIELD: {
            ...
            const value = symbolEnum.emptiness;
            const arr = new Array(Number(action.payload)).fill(new Array(Number(action.payload)).fill(value));
        ...
        case actionType.GAME_MOVE: {
            ...
            const newfieldValue = JSON.parse(JSON.stringify(state.playingField));
            const changeSymbol = state.isFirstPlayer ? state.playerFirstSymbol : state.playerSecondSymbol;
        ...
        case actionType.SETTING_PLAYER_SYMBOL: {
            ...
            const second = action.payload === symbolEnum.cross ? symbolEnum.zero : symbolEnum.cross;
            return {
